...
def plot_room(room_sz: Union[List[float], np.ndarray], pos_src: np.ndarray, pos_rcv: np.ndarray, pos_noise: np.ndarray, saveto: str = None) -> None:
    ...
    ax = Axes3D(fig)
    fig.add_axes(ax)
    ax.scatter(pos_rcv[:, 0], pos_rcv[:, 1], pos_rcv[:, 2], label='mics')
    # if len(pos_rcv) > 2:
    ...
    for i in range(len(pos_src)):
        ax.scatter(pos_src[:, 0], pos_src[:, 1], pos_src[:, 2], label=f'Source {i}')
    if pos_noise is not None and len(pos_noise) > 0:
        ax.scatter(pos_noise[:, 0], pos_noise[:, 1], pos_noise[:, 2], label=f'Noise')
...
def generate_rir_cfg_list(
    ...
    RT60_lim: Tuple[float, float] = (0.2, 0.6),
    rir_nums: Tuple[int, int, int] = (500, 500, 0),
    arr_geometry: Union[Literal['circular', 'circular+cm', 'linear', 'chime3', 'libricss'], str, List[str]] = ['linear', 'circular', 'circular+cm'],
    arr_radius: Optional[Union[Tuple[float, float], List[Tuple[float, float]]]] = (0.15/2, 0.25/2),
    ...
    mic_num: Union[int, List[int], Tuple[Union[int, List[int]], int]] = [8, 8, 8],
    mic_pos_var: float = 0,
    spk_arr_dist: Union[Tuple[float, float], Literal['auto', 'random']] = 'random',
